...
option_list = list(
  ...
  make_option("--gff", type = "character", default = NULL, action = "store",
              help = "Path or url to gff file"),
...
generate_config <- function(out_file){
  df <- data.frame(organism = "Organism name",
                   circular_chromosome = "logical T/F",
                   input_file = "location to fasta file",
                   find_gRNA_with_cutsites = "logical T/F",
                   find_paired_gRNA = "logical T/F",
                   BSgenome = "Name of the BSgenome object (BSgenome.Mtuberculosis.VUmc.H37Rv for H37)",
...
create_gRNA_library <- function(config_file, gff_file){
  ...
  txdb_file <- create_txdb(path_to_gff = gff_file,
