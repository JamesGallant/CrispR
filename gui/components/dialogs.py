...
from gui.components.workers import CustomSQL_worker
...
class ExportGuideRNA(QtWidgets.QDialog):
    ...
    def export_controls(self):
        ...
        grid = QtWidgets.QGridLayout()
        grid.addWidget(self.dataframes, 0, 0)
        groupbox.setLayout(grid)
        return groupbox
    ...
    def accept(self):
        ...
        if files_to_export == "All":
            self.candidates.to_csv(os.path.join(destination, "candidates.txt"), header=True, index=False, sep=",")
            self.backup.to_csv(os.path.join(destination, "Backup.txt"), header=True, index=False, sep=",")
            self.dropped.to_csv(os.path.join(destination, "Dropped.txt"), header=True, index=False, sep=",")
            self.offtargets.to_csv(os.path.join(destination, "Offtargets.txt"), header=True, index=False, sep=",")
...
class SearchGrnaDialog(QtWidgets.QDialog):
    ...
    def controls(self):
        ...
        grid.addWidget(self.mismatch_label, 0, 1)
        grid.addWidget(self.PAM_input, 1, 0)
        grid.addWidget(self.mismatch, 1, 1)
        grid.addWidget(self.circ_chromosome_label, 2, 0)
        grid.addWidget(self.search_databases_label, 2, 1)
        grid.addWidget(self.circ_chromosome, 3, 0)
        grid.addWidget(self.search_databases, 3, 1)
        groupBox.setLayout(grid)
        return groupBox

    def out(self):
        return {
            'pam': self.PAM_input.text(),
