...
source(paste0(getwd(), "/src", "/R", "/install_bioconductor.R"))
option_list = list(
...
create_txdb <- function(path_to_gff, organism, tax_id, circular_chroms, bsgenome_info){
...
create_gRNA_library <- function(config_file){
  ...
  REpatternFile <- paste0(script.dirname, '\\', 'NEBenzymes.fa')
  ...
  offTargetAnalysis(inputFilePath = config[["input_file"]], 
                    ...
                    n.cores.max = config[["n.cores"]],
                    allowed.mismatch.PAM = nchar(config[["PAM_sequence"]]),
